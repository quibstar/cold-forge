...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/cold_forge ./

# Uploaded logos mount here. The directory has to exist in the image and be
# owned by the runtime user: Docker seeds a fresh named volume from the image's
# directory, ownership included, so without this the volume comes up root-owned
# and every upload fails on a permission error. It cannot live under `priv` —
# a release stores that beneath a versioned path that changes on every deploy,
# which would take the branding out of mail already sent.
